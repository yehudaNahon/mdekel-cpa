...
export const onRequest = defineMiddleware((context, next) => {
	...
	if (pathname === '/' || LEGACY_PATH.test(pathname)) {
		return context.redirect(`/${preferredLocale(context.request)}${pathname}`, 302);
	}
	return next();
});
